...
def invoice_pdf(invoice_id):
    ...
    customer_gemi = saved_client.gemi_number if saved_client else ""
    ...
    if invoice.invoice_type not in {"11.1", "11.2"}:
        ...
        customer_vat_gemi = " · ".join(part for part in [f"ΑΦΜ: {invoice.vat_number}", f"Αρ. ΓΕΜΗ: {customer_gemi}" if customer_gemi else ""] if part)
        canvas.drawString(54, 570, customer_vat_gemi)
    ...
    if viva_payment:
        try:
            ...
            viva_lines = ["Πληρωμή με Viva.com", f"Transaction ID: {viva_event.get('TransactionId', viva_payment.transaction_id)}", f"TID: {viva_event.get('TerminalId', viva_payment.terminal_id)} · RRN: {viva_event.get('RetrievalReferenceNumber', '-')}", f"Order Code: {viva_event.get('OrderCode', '-')}", f"Card Number: {viva_event.get('CardNumber', '-')} · Authorization ID: {viva_event.get('AuthorizationId', '-')}"]
            pdf_notes = (pdf_notes + "\n" if pdf_notes else "") + "\n".join(viva_lines)
