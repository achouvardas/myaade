...
def invoice_pdf(invoice_id):
    ...
    if invoice.qr_url:
        widget = qr.QrCodeWidget(invoice.qr_url); left, bottom, right, top = widget.getBounds(); size = 94; drawing = Drawing(size, size); drawing.add(widget); drawing.transform = [size / (right-left), 0, 0, size / (top-bottom), 0, 0]; renderPDF.draw(drawing, canvas, 42, 85); canvas.setFillColor(cyan); canvas.drawString(150, 143, "Άνοιγμα παραστατικού ΑΑΔΕ"); canvas.setFillColor(slate); canvas.setFont("SiraSans", 8); canvas.drawString(150, 128, invoice.qr_url[:58]); canvas.linkURL(invoice.qr_url, (42, 85, 310, 180), relative=0)
